...
export const RelatedProductsCard: React.FC<{
  doc: Product
  showCategories?: boolean
  price?: number
  ...
  const {
    doc,
    doc: { slug, title, categories, photos },
    price,
    discountPercentage = 0,
  } = props

  const metaImage = photos?.length ? photos.map(item => item.photo) : []
  const adjustedPrice = parsePrice(price, discountPercentage)
  ...
  return (
    <Link href={href} className={classes.card}>
      <div className={classes.mediaWrapper}>
        ...
        {discountPercentage > 0 && (
          <div className={classes.discountTag}>{`-${discountPercentage}%`}</div>
        )}
      </div>
